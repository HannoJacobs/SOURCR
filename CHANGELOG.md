...
- Fixed pull requests not appearing next to branches, which was two compounding query mistakes. `Ref.associatedPullRequests` is a *computed* association that lags badly for recent PRs — verified live, where two PRs merged minutes earlier reported `totalCount: 0` on their own branch refs while months-old PRs resolved fine — and the query also filtered to `states: OPEN`, so merged PRs were excluded even when the association did resolve. Branch PRs now come from the repository's own pull request list mapped by `headRefName`, newest-updated first, which is what GitHub's Branches page shows.
- The PR pill now carries state, not just a draft flag: green open, grey draft, purple merged, red closed, each with its own icon (`arrow.triangle.pull` / `arrow.triangle.merge` / `xmark.circle`). Clicking it opens the PR in the browser; hovering shows its title. A closed draft reads as closed and a merged PR is never reported as draft, matching GitHub.
- A branch deleted on the remote now drops off the board even while its workflow runs remain in GitHub's history for days, because rows are built from live refs rather than from run history. Covered by a regression test and confirmed live when a merged PR's branch was auto-deleted.
